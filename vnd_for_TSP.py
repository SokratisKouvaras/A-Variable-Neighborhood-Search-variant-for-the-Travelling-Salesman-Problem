...
import numpy as np
import random
#import urllib # for future work on online reading the TSP files
...
class VND :
    def __init__(self,filepath):
        self.stops = self.parse(filepath)
    ...
    def parse(self,filepath):    
        ...
        for item in cleaned:
            if item.startswith("DIMENSION"):
                N=int(re.compile(r'[^\d]+').sub("",item))                                                    #τη λέξη DIMENSION κρατάει το πλήθος                                                  #των πόλεων.                    
        for item in cleaned:
